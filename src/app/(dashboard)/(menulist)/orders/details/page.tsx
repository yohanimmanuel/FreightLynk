...
const ClientDetailsUI = () => {
  return (
    <div className="p-4">
      ...
      <p className="text-gray-600">Purchase order details coming soon...</p>
    </div>
  );
};
...
const ForwarderDetailsUI = () => {
  ...
  const transformItem = (item: PODetail): POItem => {
    ...
    return {
      ...
      bookedQty: item.booked || 0,
      bookingProgress: item.requested > 0 ? Math.round((item.booked / item.requested) * 100) : 0
    };
  };
